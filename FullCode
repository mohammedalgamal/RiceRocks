...
class Sprite:
    ...
    def collide(self,obj):
        dist_diff = dist(self.get_position(),obj.get_position())
        if dist_diff <= (self.get_radius()+ obj.get_radius()):
            return True
        else :
            return False
...
def process_sprite_group(canvas, sets):
    for i in set(sets) :
        i.draw(canvas)
        if i.update():
            sets.remove(i)
...
def group_collide(group, obj):
    ...
    for i in set(group):
        if i.collide(obj):
            group.remove(i)
            collision = True
            explosion_group.add(Sprite(i.pos, i.vel, 0, 0, explosion_image, explosion_info, explosion_sound))
            #print explosion_group
    return collision        

def group_group_collide(group_1, group_2):
    #global collided
    for i in set(group_1):
...
def draw(canvas):
    global time, started, rock_group, lives, missile_group, score, soundtrack
...
def rock_spawner():
    ...
    if started :
        ...
        rock_vel = [random.random() * .6 - .3 * score * 0.07, random.random() * .6 - .3 * score * 0.07]
        rock_avel = random.random() * .2 - .1   
        if (len(rock_group)< 12) and (dist(rock_pos, my_ship.pos) > my_ship.get_radius() + 50) :
